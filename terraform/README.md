...
- SNS topic and email subscription for Lambda failure alerts.
- CloudWatch alarm on Lambda Errors that publishes to SNS.
...
- alert_email: recipient for SNS failure notifications.
...
- youtube_alerts_topic_arn
- youtube_lambda_errors_alarm_name
...
- After first apply, AWS sends an SNS subscription confirmation email to alert_email; notifications will not be delivered until the link is confirmed.
